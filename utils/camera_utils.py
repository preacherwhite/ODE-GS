...
def loadCam(args, id, cam_info, resolution_scale):
    # Handle case where image is None (training images skipped)
    if cam_info.image is None:
        # Use width/height from cam_info if available, otherwise use defaults
        orig_w = cam_info.width if hasattr(cam_info, 'width') else 800
        orig_h = cam_info.height if hasattr(cam_info, 'height') else 600
        gt_image = None
        loaded_mask = None
    else:
...
def cameraList_from_camInfos(cam_infos, resolution_scale, args):
    ...
    for id, c in enumerate(tqdm(cam_infos, desc="Loading camera infos")):
