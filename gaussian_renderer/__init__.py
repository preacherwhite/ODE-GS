...
def render_ode(
    ...
    if is_6dof:
        ...
        means3D = new_xyz
    #means2D = screenspace_points
    opacity = new_opacity if new_opacity is not None else pc.get_opacity
    ...
    shs = None
    colors_precomp = None
    feature_source = new_features if new_features is not None else pc.get_features
    if colors_precomp is None:
        if pipe.convert_SHs_python:
            shs_view = feature_source.transpose(1, 2).view(-1, 3, (pc.max_sh_degree + 1) ** 2)
            dir_pp = (pc.get_xyz - viewpoint_camera.camera_center.repeat(feature_source.shape[0], 1))
            dir_pp_normalized = dir_pp / dir_pp.norm(dim=1, keepdim=True)
            sh2rgb = eval_sh(pc.active_sh_degree, shs_view, dir_pp_normalized)
            colors_precomp = torch.clamp_min(sh2rgb + 0.5, 0.0)
        else:
